...
public struct LineChart: View {
    ...
    public init(data: [TimeSeries], configuration: ChartConfiguration = ChartConfiguration()) {
        self.data = data
        _presenter = State(initialValue: ChartPresenter(data: data, configuration: configuration, traits: ChartPresenter.Traits(showsBand: true)))
    }

    public var body: some View {
        TimeSeriesChart(presenter: presenter, data: data) {
            SeriesBands(points: presenter.bandPoints)
            SeriesLines(points: presenter.linePoints)
...
#Preview {
    LineChart(data: TimeSeries.samples())
}
